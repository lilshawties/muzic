...
const { showplaylist: lang } = require('../loadlanguage.js'); 
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    db.all('SELECT * FROM playlists WHERE owner = ?', [userId], async (err, rows) => {
      if (err) {
          ...
          .setDescription(
            rows.slice(i * playlistsPerEmbed, (i + 1) * playlistsPerEmbed)
                .map(playlist => {
                  ...
                  return `**${playlist.name}** (${songCount} ${lang.songs}) - ${lang.visibility}: ${playlist.isPublic ? lang.public : lang.private}`;
                })
                .join('\n')
          );
